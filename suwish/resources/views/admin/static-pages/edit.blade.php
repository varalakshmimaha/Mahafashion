        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <select name="category" id="category" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <option value="quick_link" {{ old('category', $page->category) == 'quick_link' ? 'selected' : '' }}>Quick Link</option>
                    <option value="policy" {{ old('category', $page->category) == 'policy' ? 'selected' : '' }}>Policy</option>
                </select>
                @error('category')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
